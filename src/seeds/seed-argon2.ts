/**
 * argon2 비교 테스트용 더미 데이터 시딩 스크립트
 * 실행: npm run seed:argon2
 *
 * seed.ts와 동일한 데이터 구조 및 개수, 비밀번호 해싱만 argon2id로 교체
 *
 * 삽입 순서
...
async function seed() {
  ...
  console.log('  Academy 더미 데이터 시딩 시작 (argon2)');
  console.log('━'.repeat(60));
  ...
  const firstStudentId = await insertBatch(
    ...
    Array.from({ length: 200 }, (_, i) => ({
      ...
      parentId: i < 100 ? firstParentId + i : null,
      grade: (i % 3) + 1,
      school: schools[i % 5],
    })),
  );
  ...
  for (let planIdx = 0; planIdx < wrongPlans.length; planIdx++) {
    ...
    if (wrongDetailOffsets.length === 0) continue;
